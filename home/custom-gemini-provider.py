...
import os
import asyncio
from typing import ClassVar, List, Optional, AsyncIterator, Dict, Any
...
class CustomGeminiProvider(BaseProvider, Runnable):
    ...
    models: ClassVar[List[str]] = ["gemini-2.5-flash", "gemini-1.5-pro-latest"] # Add supported models
    auth_strategy: ClassVar[Optional[str]] = None # Assuming API key is handled by env var
    ...
    async def predict(self, model_id: str, prompt: str, *args, **kwargs) -> Generation:
        ...
        try:
            ...
            response = await asyncio.wait_for(self.invoke({"prompt": prompt}), timeout=10.0)
